...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  function animate() {
    ...
    let surge = wave3 * 1.5 + wave4 * 2.0;
    
    // Inject LIS state into the visualization
    let targetBloom = 1.8;
    if (currentState === "listening") {
        ...
        surge += 0.5 + Math.sin(t * 10) * 0.5;
        targetBloom = 2.0;
        outerShell.rotation.x += 0.005;
        panelGroup.rotation.z += 0.005;
    } else if (currentState === "speaking") {
        surge += audioPulse * 3.0; // Audio reactive surge!
        targetBloom = 1.5 + (audioPulse * 2.0);
        outerShell.rotation.y += 0.005;
    }
    
    // Smooth bloom transition
    bloom.strength += (targetBloom - bloom.strength) * 0.1;
