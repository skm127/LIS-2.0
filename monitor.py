"""
LIS Conversation Monitor

Watches the LIS server logs in real-time, analyzes conversation quality,
and reports issues that need fixing. Run alongside the LIS server.
...
class ConversationMonitor:
    ...
    def analyze_latest(self):
        ...
        if latest["role"] == "lis":
            ...
            if len(sentences) > 4:
                self.flag(f"LIS response too long for voice ({len(sentences)} sentences): {text[:80]}...")

            # Generic AI patterns that LIS shouldn't use
            bad_patterns = [
                ("How can I help", "LIS doesn't ask 'how can I help' — he just acts"),
                ("Is there anything else", "LIS doesn't ask 'is there anything else'"),
                ("I'd be happy to", "Too corporate — LIS says 'Will do, sir' or just does it"),
                ("Absolutely!", "LIS doesn't use filler enthusiasm"),
                ("Great question", "LIS never says 'great question'"),
                ("I don't have access", "LIS should say 'I'm afraid I don't have that information, sir'"),
                ("As an AI", "LIS never breaks character"),
                ("I cannot", "LIS says 'I'm afraid that's beyond my current capabilities, sir'"),
            ]
            ...
            lis_msgs = [m for m in self.messages if m["role"] == "lis"]
            if len(lis_msgs) >= 5:
                recent = lis_msgs[-5:]
                sir_count = sum(1 for m in recent if "sir" in m["text"].lower())
                if sir_count < 1:
                    self.flag("LIS hasn't said 'sir' in the last 5 responses — should use it more")
            ...
            if prev and prev["role"] == "user":
                ...
                if any(w in user_text for w in ["earlier", "before", "you said", "we talked about", "remember"]):
                    if "I don't recall" in text or "I'm not sure what" in text:
                        self.flag("LIS failed to recall earlier conversation — memory issue")

            # Response references Samantha?
            if "samantha" in text.lower():
                self.flag("LIS referenced 'Samantha' — should never mention her, he IS the assistant")
        ...
        if latest["role"] == "user":
            ...
            for pattern in complaint_patterns:
                if pattern in text:
                    self.flag(f"USER COMPLAINT detected: '{pattern}' — review LIS's previous response")
...
def main():
    ...
    print("🔍 LIS Conversation Monitor")
    ...
    try:
        for line in sys.stdin:
            ...
            # Parse LIS responses
            lis_match = re.search(r"LIS: (.+)$", line)
            if lis_match:
                text = lis_match.group(1)
                print(f"🤖 {text[:80]}{'...' if len(text) > 80 else ''}")
                monitor.add_message("lis", text)
