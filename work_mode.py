"""
LIS Work Mode — persistent claude -p sessions tied to projects.

LIS can connect to any project directory and maintain a conversation
...
LIS reads the responses via subprocess, summarizes, and reports back.
"""
...
# Sentinel returned when claude CLI is dead so caller can detect & fallback
CLI_DEAD_SENTINEL = "__CLI_DEAD__"


class WorkSession:
    """A claude -p session tied to a project directory.

    Each project gets its own session. LIS can switch between projects
    ...
    async def send(self, user_text: str) -> str:
        ...
        Returns CLI_DEAD_SENTINEL if the CLI is broken so caller can fallback.
        """
        # Circuit breaker — don't keep hitting a dead CLI
        if self._cli_dead:
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                self._status = "error"
                # Trip the circuit breaker — stop retrying a broken CLI
                self._cli_dead = True
                log.warning("Claude CLI marked dead for this session. Work mode will use Groq fallback.")
                return CLI_DEAD_SENTINEL
